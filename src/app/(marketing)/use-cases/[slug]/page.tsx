...
export async function generateStaticParams() {
  return getAllContent('use-cases').map(p => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const item = getContentBySlug('use-cases', slug)
  if (!item) return {}
  return {
...
export default async function UseCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      <Link href="/use-cases" className="text-[12px] text-muted hover:text-ink transition-colors mb-8 block">
        ← All use cases
      </Link>

      <div className="mb-10">
        {item.frontmatter.category && (
          <span className="text-[11px] rounded-full bg-surface-2 border border-border px-2.5 py-1 text-muted mb-4 inline-block">
            {item.frontmatter.category}
          </span>
        )}
