  ...
  return (
    <div className="min-h-screen bg-paper font-body">
      ...
      <section className="mx-auto max-w-5xl px-6 py-20">
        <p className="label-section mb-3">The product</p>
        <h1 className="font-display text-4xl text-ink leading-tight mb-6 max-w-3xl md:text-5xl">
        ...
        <p className="mt-4 text-xs text-muted">14-day trial · Card required · Cancel anytime.</p>
      </section>

      {/* ── Intelligence layer (the moat) ── */}
      <section className="border-t border-border bg-surface">
        <div className="mx-auto max-w-5xl px-6 py-20">
          <p className="label-section mb-3">What makes Mayil different</p>
          <h2 className="font-display text-2xl text-ink mb-4">Three engines, one brief.</h2>
          <p className="text-[14px] text-muted max-w-2xl mb-12 leading-relaxed">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {INTELLIGENCE.map(item => (
              <div key={item.label} className="rounded-[10px] border border-border bg-paper p-6">
                <h3 className="text-sm font-semibold text-ink mb-3">{item.label}</h3>
      ...
      <section className="border-t border-border bg-paper">
        <div className="mx-auto max-w-5xl px-6 py-20">
          <p className="label-section mb-3">How the brief is built</p>
          <h2 className="font-display text-2xl text-ink mb-4">From snapshot to inbox in eleven hours.</h2>
          <p className="text-[14px] text-muted max-w-2xl mb-12 leading-relaxed">
            ...
            {PIPELINE.map((p, i) => (
              ...
              >
                <div className="flex items-center gap-4 sm:w-48 shrink-0">
                  <span className="font-mono text-[11px] text-muted shrink-0 w-16">{p.time}</span>
                  <h3 className="text-sm font-semibold text-ink">{p.stage}</h3>
                </div>
                <p className="text-[13px] text-muted leading-relaxed flex-1">{p.body}</p>
              </div>
            ))}
          </div>
          <p className="mt-4 text-xs text-muted">All times Indian Standard Time. Briefs are delivered in the recipient&apos;s timezone with consistent Sunday-morning timing wherever you are.</p>
        </div>
      </section>
      ...
      <section className="border-t border-border bg-surface">
        <div className="mx-auto max-w-5xl px-6 py-20">
          <p className="label-section mb-3">Channels</p>
          <h2 className="font-display text-2xl text-ink mb-4">What we collect — honestly.</h2>
          <p className="text-[14px] text-muted max-w-2xl mb-12 leading-relaxed">
          ...
          <h3 className="text-sm font-semibold text-ink mb-4">Live in every brief</h3>
          <div className="rounded-[10px] border border-border bg-paper overflow-hidden mb-10">
            {CHANNELS_LIVE.map((c, i) => (
              ...
              >
                <h4 className="text-sm font-semibold text-ink sm:w-56 shrink-0">{c.name}</h4>
                <p className="text-[13px] text-muted leading-relaxed flex-1">{c.detail}</p>
              </div>
            ))}
          </div>

          <h3 className="text-sm font-semibold text-ink mb-4">In development</h3>
          ...
          <p className="mt-4 text-xs text-muted">When a new channel ships, every existing customer gets it. No upgrade required.</p>
        </div>
      </section>
      ...
      <section className="border-t border-border bg-paper">
        <div className="mx-auto max-w-5xl px-6 py-20">
          <p className="label-section mb-3">A sample brief</p>
          <h2 className="font-display text-2xl text-ink mb-12">What lands in your inbox.</h2>
          <div className="mx-auto max-w-2xl rounded-[14px] border border-border bg-surface shadow-card overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              <div className="signal-bar-threat rounded-r-[10px] bg-[#FDECEA] p-4">
                ...
                <p className="text-sm font-semibold text-ink">HubSpot launched 23 new Meta ads — 4.6× the panel median across 6 tracked competitors.</p>
                <p className="text-[13px] text-muted mt-1">All target the SMB segment. Creative theme is consolidation: &quot;one platform replaces five.&quot; Largest paid push since their Q4 launch.</p>
              </div>
              <div className="signal-bar-opportunity rounded-r-[10px] bg-[#EBF7EE] p-4">
                ...
                <p className="text-sm font-semibold text-ink">Salesforce paused all branded search ads for the third week running.</p>
                <p className="text-[13px] text-muted mt-1">Branded query coverage on the panel dropped to 0. Bid on their brand terms now while CPC is uncontested.</p>
              </div>
              <div className="signal-bar-watch rounded-r-[10px] bg-[#FBF5E4] p-4">
                ...
                <p className="text-sm font-semibold text-ink">Zendesk appeared in 14 articles this week — every one mentions an AI-agent pivot.</p>
      ...
      <section className="border-t border-border bg-surface">
        <div className="mx-auto max-w-5xl px-6 py-20">
          <p className="label-section mb-3">Data handling &amp; privacy</p>
          <h2 className="font-display text-2xl text-ink mb-4">We collect what&apos;s public. We protect what&apos;s yours.</h2>
          <p className="text-[14px] text-muted max-w-2xl mb-12 leading-relaxed">
          ...
          <div className="grid gap-6 md:grid-cols-2 mb-12">
            <div className="rounded-[10px] border border-border bg-paper p-6">
              <h3 className="text-sm font-semibold text-ink mb-2">Where data lives</h3>
            ...
            <div className="rounded-[10px] border border-border bg-paper p-6">
              <h3 className="text-sm font-semibold text-ink mb-2">Retention</h3>
              <p className="text-[13px] text-muted leading-relaxed">
            ...
            <div className="rounded-[10px] border border-border bg-paper p-6">
              <h3 className="text-sm font-semibold text-ink mb-2">GDPR &amp; privacy rights</h3>
              <p className="text-[13px] text-muted leading-relaxed">
            ...
            <div className="rounded-[10px] border border-border bg-paper p-6">
              <h3 className="text-sm font-semibold text-ink mb-2">Data Processing Agreement</h3>
              <p className="text-[13px] text-muted leading-relaxed">
          ...
          <h3 className="text-sm font-semibold text-ink mb-4">Sub-processors</h3>
          <div className="rounded-[10px] border border-border bg-paper overflow-hidden">
            {SUBPROCESSORS.map((s, i) => (
              ...
              >
                <h4 className="text-sm font-medium text-ink sm:w-32 shrink-0">{s.name}</h4>
                <p className="text-[13px] text-muted leading-relaxed flex-1">{s.purpose}</p>
              </div>
            ))}
          </div>
          <p className="mt-4 text-xs text-muted">We notify Enterprise customers in advance of any sub-processor changes.</p>
        </div>
      </section>
